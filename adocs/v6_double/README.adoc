...
TIP: 本番環境では、ユーザアカウントデータは (多くの場合、ディレクトリサービスのような) バックエンドデータベースで管理されており、Spring Configuration にハードコードされることはないでしょう。このようなデータベースに接続するサンプルアプリケーションは、例えば https://github.com/spring-projects/spring-security/tree/master/samples[Spring Security Samples]  のように、インターネット上で簡単に見つけることができます。
...
このアクセス制御は Admin アプリケーションで行います。(このバックエンド全体で必要とされる) "ADMIN" ロールに対して、以下のように Spring Security を設定します:
